...
{
	double heuristic = 0;
	if (heuristicChoice == PathfindingForm::Heuristic_Choice::Manhattan)
	{
	...
	else if (heuristicChoice == PathfindingForm::Heuristic_Choice::Chebyshev)
	{
		heuristic = max(abs(current.cell.getPosition().x - finish->cell.getPosition().x),	// Chebyshev distance
			abs(current.cell.getPosition().y - finish->cell.getPosition().y));
	}
	else if (heuristicChoice == PathfindingForm::Heuristic_Choice::Euclidean)
	{
